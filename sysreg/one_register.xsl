...
<xsl:template name="generate_footer">
  ...
  <p class="copyconf">Copyright &#169; 2010-2020 Arm Limited or its affiliates. All rights reserved. This document is Non-Confidential.</p>
</xsl:template>
...
<xsl:template match="register">
  ...
  <xsl:choose>
    ...
    <xsl:when test="./@is_internal = 'False' and ./reg_address/@power_domain = 'None' and count(./reg_mappings/reg_mapping) = 0 and count(./reg_configuration/configuration_text) = 0  and not(./reg_condition) and (not(./arch_variants/arch_variant) or starts-with(./arch_variants/arch_variant[1]/@name, 'ARMv8.0'))">
      <p>There are no configuration notes.</p>
    </xsl:when>
  </xsl:choose>
...
<xsl:template name="reg_field">
  ...
  <xsl:variable name="ID">
    <!-- Incorporate fields instance if partial fieldset-->
    <xsl:value-of select="translate($FIELD/../fields_instance/text(), ' ', '')"/>
    <!-- Incorporate the condition if there's no instance -->
    <xsl:if test="not($FIELD/../fields_instance/text()) and $FIELDAT/../fields_condition != ''">
      <xsl:value-of select="translate($FIELDAT/../fields_condition/text(),'[]= ','')"/>
    </xsl:if>
...
<xsl:template name="reg_field_array">
  ...
  <xsl:variable name="FIELD_NAME_SUFFIX">
    <xsl:value-of select="substring-after($FIELD_NAME,'&gt;')"/>
  </xsl:variable>
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      ...
      <xsl:call-template name="reg_field_single">
        ...
        <xsl:with-param name="name" select="concat($FIELD_NAME_PREFIX,$element_count_start,$FIELD_NAME_SUFFIX)"/>
      </xsl:call-template>
...
<xsl:template name="field_single">
  ...
  <xsl:variable name="ID"> <!-- Use the node's original field_name for this, not whatever we've assigned to the $field_name param. -->
    <xsl:value-of select="translate(../fields_instance/text(), ' ', '')"/>
    <xsl:if test="not(../fields_instance/text() and ../fields_condition != '')">
      <xsl:value-of select="translate(../fields_condition/text(),'[]= ','')"/>
    </xsl:if>
    <xsl:choose>
      <xsl:when test="field_name">
        <xsl:value-of select="translate(field_name,'[]= ','')"/>
...
<xsl:template name="fields_descs">
  <xsl:for-each select="field">
    <xsl:if test="field_description">
      ...
      <h4>
        <!-- Set ID if either no conditions, or this is the description under the first condition
             First condition is derived from the _1 at the end of the XML field id - this number
             goes up sequentially for conditioned fields -->
        <xsl:if test="not(./fields_condition or ../field[field_msb = $MSB and field_lsb = $LSB]/fields_condition) or '_1' = substring(@id, string-length(@id) - 1)">
    ...
    <xsl:for-each select="field_reset[not(@reset_type = 'impdef')]">
      <xsl:choose>
        <xsl:when test="field_reset_conditions">
          <xsl:for-each select="field_reset_conditions/field_reset_condition"> <!-- render each conditional reset -->
            <xsl:choose>
              <xsl:when test="field_reset_special_text">
                <xsl:apply-templates select="field_reset_special_text"/>
              </xsl:when>
              <xsl:otherwise>
                <p><xsl:call-template name="field_reset_condition"/></p>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
        </xsl:when>
        <xsl:otherwise> <!-- render each unconditional reset -->
          <xsl:choose>
            <xsl:when test="field_reset_special_text">
              <xsl:apply-templates select="field_reset_special_text"/>
            </xsl:when>
            <xsl:otherwise>
              <p><xsl:call-template name="field_reset"/></p>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:otherwise>
      </xsl:choose>
...
<xsl:template name="field_reset"> <!-- unconditional field reset -->
  ...
  <xsl:choose>
    <xsl:when test="not(field_reset_standard_text) or field_reset_standard_text != 'UC'">
      ...
      <xsl:text>field resets</xsl:text>
...
<xsl:template name="field_reset_condition"> <!-- conditional field reset -->
  <xsl:variable name="HAS_RESET_TYPE">
    <xsl:choose>
